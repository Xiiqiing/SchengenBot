...
export function PageHeader({ title, description, icon, backHref, backLabel = "返回" }: PageHeaderProps) {
    ...
    return (
        <header ref={headerRef} className="bg-transparent pb-6 pt-8 transition-opacity duration-300 md:pb-8 md:pt-10">
            <div className="container mx-auto px-4 max-w-6xl">
                <div className="space-y-4">
                    ...
                    <div className="max-w-3xl">
                        {icon && (
                            <div className="mb-3 flex h-9 w-9 items-center justify-center rounded-full bg-white/80 shadow-[0_10px_24px_rgba(15,23,42,0.06)]">
                                {icon}
                            </div>
                        )}
                        <h1 className="text-[32px] font-semibold tracking-[-0.045em] text-[#1d1d1f] md:text-[46px]">
                            {title}
                        </h1>
                        {description && <p className="mt-2 text-[15px] leading-7 text-[#6e6e73] md:text-[17px]">{description}</p>}
                    </div>
                </div>
            </div>
        </header>
